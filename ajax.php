...
else if (isset($_POST['gust_change_password'])) {
    $new = $_POST['password_new'];
    $old = $_POST['password_current'];
    $userName=$_SESSION['username'];
    if(!$new){
        header('Location:index.php?custom_password_change&empty');
    }
            $password = md5($old);
            $newpassword = md5($new);
            ...
            if (mysqli_num_rows($result)> 0) {
                ...
                if( $result1){
                    header('Location:index.php?custom_password_change&success');
                }
                else{
                    header('Location:index.php?custom_password_change&dberror');
                }
             }
                else{
    
                    header('Location:index.php?custom_password_change&error');
                }
...
$userName=$_SESSION['username'];
if(!$new){
    header('Location:index.php?change_password&empty');
}
        $password = md5($old);
        $newpassword = md5($new);
        $query = "SELECT * FROM user WHERE username = '$userName' AND password='$password'";
        ...
        if (mysqli_num_rows($result)> 0) {
            ...
            $result1 = mysqli_query($connection, $query1);
            if( $result1){
                header('Location:index.php?change_password&success');
            }
            else{
                header('Location:index.php?change_password&dberror');
            }
         }
            else{

                header('Location:index.php?change_password&error');
            }
...
else if (isset($_GET['delete_room'])) {
...
else if (isset($_POST['room_price'])) {
    $room_id = $_POST['room_id'];
...
else if (isset($_POST['custom_reserve'])) {
    ...
    $room_type = $_POST['room_type1'];
    $date_diff = $_POST['date_diff'];
    if(!$room_type){
    echo "no type";
    return false;
    }
    $sqlprice = "SELECT * FROM room_type WHERE room_type_id='$room_type' ";
    $resultprice = mysqli_query($connection, $sqlprice);
    if( $resultprice->num_rows > 0){
        $room = mysqli_fetch_assoc($resultprice); 
        $total_price=$room['price']*$date_diff;
       }

    $sql = "SELECT * FROM room WHERE room_type_id='$room_type' and status IS NULL AND deleteStatus = '0'";
    $result = mysqli_query($connection, $sql);
    if($result->num_rows > 0){
     ...
     header('Location:home.php?booking&empty');
     return false ;
    }
    $customer_sql = "INSERT INTO customer (customer_name,contact_no,email,id_card_type_id,id_card_no) VALUES ('$name','$contact_no','$email','1','000000000000')";
    ...
    if ($customer_result) {
        ...
        $user_sql = " INSERT INTO user(name,username,password) VALUES ('$name','$contact_no','$password')  ";
        ...
        if ($booking_result) {
            ...
            if (mysqli_query($connection, $room_stats_sql)) {
                ...
                header('Location:home.php?booking&dberror');
                return false ;
              }
        }
    }
   ...
   echo json_encode($response);
...
else if (isset($_POST['cutomerDetails'])) {
    //$customer_result='';
    $room_id = $_POST['room_id'];
...
else if (isset($_POST['booked_room'])) {
    $room_id = $_POST['room_id'];
...
else if (isset($_POST['check_in_room'])) {
...
else if (isset($_POST['createComplaint'])) {
    ...
    $result = mysqli_query($connection, $query);
    if ($result) {
        header("Location:index.php?custom_complain&success");
    } else {
        header("Location:index.php?custom_complain&error");
    }
    ...
    $NoItem = $_POST['item_no'];
    $roomNo = $_POST['room_no'];
    $phoneNo = $_POST['phone_no'];
    $customerId=$_SESSION['username']; 

    $query = "INSERT INTO requests(customerId, roomid,Item,NoItem,contact_no) VALUES ('$customerId',' $roomNo','$Item_name','$NoItem','$phoneNo')";
    $result = mysqli_query($connection, $query);
    if ($result) {
        header("Location:index.php?food_order&success");
    } else {
        header("Location:index.php?food_order&error");
    }

}

else if (isset($_POST['resolve_complaint'])) {
    $budget = $_POST['budget'];
...
else if (isset($_POST['order_food'])) {
    $NoItem = $_POST['budget'];
    $complaint_id = $_POST['complaint_id'];
    $Item="Tibse";
    $query = "INSERT INTO requests(customerId,roomid,Item,NoItem,totalPrice) VALUES (7,1,'$Item','$NoItem',230)";
    $result = mysqli_query($connection, $query);
    if ($result) {
        header("Location:index.php?food_order&resolveSuccess");
    } else {
        header("Location:index.php?food_order&resolveError");
    }
}

else if (isset($_POST['accept_request'])) {
    $casher = $_POST['casher'];
    $complaint_id = $_POST['request_id'];
   
    $query = "UPDATE requests set ResolvedBy = '$casher',status = '1' WHERE requestId='$complaint_id'";
    $result = mysqli_query($connection, $query);
    if ($result) {
        header("Location:index.php?customer_Request&resolveSuccess");
    } else {
        header("Location:index.php?customer_Request&resolveError");
    }
}



else if (isset($_POST['change_shift'])) {
    $emp_id = $_POST['emp_id'];
